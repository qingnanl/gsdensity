...
# we first need to calculate the pathway activities for random gene sets, for controling 
# the estimation of statistical significance
# the parameter 'n.times' is the number of random gene sets generated for each real gene set 
# (and approximates the gene set size). Here we use 10 for quick running purposes. In real analysis,
# we recommend increase this to 50-100
random.activities <- compute.random.pal.list(gene.set.list = gene.set.list[gene.set.deviated.2], 
                                             n.times = 10, gene.use = rownames(brain), el = el, 
                                             cells = cells)

# the 'weight_df' should have a format as the output of 'run.rwr.list'; here we use the terms with at least 100 positive cells
# the r.df is an output of 'compute.random.pal.list'. The gene set list used for running 'compute.random.pal.list' must
# match the gene set used for spatial KLD calculation (order also need to be the same)
...
spatial.klds <- compute.spatial.kld.df(spatial.coords = coords.df, r.df = random.activities,
...
top.spatial.terms <- rownames(spatial.klds[spatial.klds$p.value < quantile(spatial.klds$p.value, 0.2), ])
top.spatial.terms
...
brain@meta.data$GOBP_ENSHEATHMENT_OF_NEURONS <- cv.df[rownames(brain@meta.data), 
                                                           "GOBP_ENSHEATHMENT_OF_NEURONS"]

SpatialFeaturePlot(brain, features = c("GOBP_ENSHEATHMENT_OF_NEURONS")) + 
...
brain@meta.data$GOBP_APOPTOTIC_MITOCHONDRIAL_CHANGES <- cv.df[rownames(brain@meta.data), 
                                                           "GOBP_APOPTOTIC_MITOCHONDRIAL_CHANGES"]

SpatialFeaturePlot(brain, features = c("GOBP_APOPTOTIC_MITOCHONDRIAL_CHANGES")) + 
        theme(legend.position = "top")
...
# add the label propagation probability to metadata for cluster_1 specific terms
brain@meta.data$GOBP_DOPAMINERGIC_NEURON_DIFFERENTIATION <- cv.df[rownames(brain@meta.data), 
                                                           "GOBP_DOPAMINERGIC_NEURON_DIFFERENTIATION"]

SpatialFeaturePlot(brain, features = c("GOBP_DOPAMINERGIC_NEURON_DIFFERENTIATION")) + 
        theme(legend.position = "top")
