...
gene.set.list.df <- data.frame(gene.set = names(gene.set.list), genes = genes)
...
# By default, all genes from the single cell data is used; alternatively, we can use only the genes appearing in 
# the gene sets; a sufficient number of genes should be retained (a few thousands) for the purpose of calculating
# the background; can be adjusted by setting genes.use
ce <- compute.mca(object = pbmc3k)
head(ce)

# Compute the deviation of each gene list
...
# At this stage, we know that for all the B cell related GO terms, these 13 demonstrated some patterns/specificity in out dataset. We do not need to know anything about the dataset itself (e.g., clustering information). Now, we can start from whichever term to fetch cells the most relevant to it.
...
# Plot (this pbmc3k data is pre-annotated and the 'seurat_annotations' comes with the SeuratData)
